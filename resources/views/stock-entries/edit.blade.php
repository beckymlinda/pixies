                ...
                <div class="table-responsive">
                    <table class="table stock-table mb-0">
                        ...
                        <tbody id="sellTableBody">
                            @include('stock-entries._edit_rows')
                        </tbody>
                    </table>
                </div>
            </div>
...
// Live reload: quietly re-fetch this sheet's rows every few seconds so a
// director's price/stock/name change shows up without a manual refresh.
// Only ever patches a row that's currently safe to touch - never one with
// an unsaved sales quantity mid-typing (or focused, or a pending "clear
// sales" not yet saved) - so a seller's in-progress entry is never lost.
(function () {
    const POLL_INTERVAL_MS = 8000;
    const pollUrl = new URL(window.location.href);
    pollUrl.searchParams.set('poll', '1');

    function rowIsSafeToRefresh(row) {
        const salesInput = row.querySelector('.new-sales');
        if (salesInput) {
            if (document.activeElement === salesInput) return false;
            if ((parseFloat(salesInput.value) || 0) > 0) return false;
        }
        const clearInput = row.querySelector('.clear-sales');
        if (clearInput && clearInput.value === '1') return false;
        return true;
    }
    ...
    function poll() {
        fetch(pollUrl.toString(), { headers: { 'Accept': 'text/html' } })
            ...
            .then(html => {
                ...
                document.querySelectorAll('#sellTableBody tr[data-item-id]').forEach(oldRow => {
                    if (!rowIsSafeToRefresh(oldRow)) return;

                    const itemId = oldRow.getAttribute('data-item-id');
                    const newRow = fresh.querySelector(`tr[data-item-id="${itemId}"]`);
                    if (!newRow) return; // item no longer in this bar's list - leave as-is
